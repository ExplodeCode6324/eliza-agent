...
| 外部依赖 | chromedp v0.14.2, golang.org/x/term v0.44.0, creack/pty v1.1.24 |
...
基于 Go `chromedp` v0.14.2 内置完整的无头浏览器控制层，不依赖 Node、Python、Playwright 或 Puppeteer。Chromium 本体为可选资产——解压到二进制同目录 `tools/` 即自动激活。
...
### 4. 工具注册表标准化 (ToolRegistry)

将所有工具重构为统一接口体系，解决之前注册方式不统一、权限检查分散、审批触发逻辑耦合在各自文件中的问题。

**新增 Tool 接口体系（8 个接口）：**

| 接口 | 用途 |
|------|------|
| `Tool` | 基础接口：`Definition()` + `Execute()` |
| `ContextTool` | 支持 context 传递的工具执行 |
| `ToolSpecProvider` | 工具声明类别/只读/审批策略/Profile |
| `ToolArgumentValidator` | 工具自定义参数校验 |
| `ToolAuthorizer` | 工具自定义权限检查（FilePolicy 等） |
| `ToolApprovalRequester` | 工具自定义审批触发逻辑 |
| `ToolResultFormatter` | 工具自定义输出格式化 |
| `ToolAuditContextSetter` | 工具接收审计上下文（requestID/toolCallID） |

**ToolRegistry 统一管线：**
- `Register()` / `RegisterMany()` — 统一注册入口
- `Definitions()` — 获取 mode+role 过滤后的工具定义
- `ValidateCall()` — 参数 schema 校验（类型/enum/required）
- `AuthorizeCall()` — 权限前置检查（policy + role + tool authorizer）
- `ApprovalRequest()` — 统一审批触发判断
- `ExecuteContext()` — 统一执行入口（注入审计上下文）
- `FormatResult()` — 统一输出格式化

**4 种审批策略：** `never` / `always` / `dangerous_command` / `memory_mutation`

**4 种工具 Profile：** `minimal_readonly` / `code_review` / `ops` / `browser_enhanced`

**已有工具全部重构为标准接口：**
- `ReadFileTool` / `WriteFileTool` / `RunCommandTool` 均实现 `Tool` + `ToolAuthorizer`
- `RunCommandTool` 额外实现 `ToolArgumentValidator` + `ContextTool`

**新增测试：** `cmd/eliza/tool_registry_test.go` (141 行)，覆盖权限过滤、schema 校验、审批触发、审计注入。

**修改文件：** `cmd/eliza/tools.go` (+570), `cmd/eliza/tool_registry_test.go` (新增), `cmd/eliza/agent.go`, `cmd/eliza/main.go`, `cmd/eliza/memory.go`, `cmd/eliza/skill.go`, `cmd/eliza/browser.go`, `cmd/eliza/vision.go`

---

### 5. TUI 重构为事件驱动架构

将 Agent 逻辑与终端渲染器解耦，新增事件驱动层和声明式组件。

**核心变化：**
- `ui_events.go`：9 种 UIEvent（Status/Message/ToolCall 等）+ 3 种 UICommand + AgentUI 封装
- `ui_components.go`：5 种声明式组件（Message/Status/ToolCall/InputBar/Approval），每组件返回预包装行列表
- `ui.go`：精简 -85 行，业务逻辑移到组件中
- 新增 PTY 真实终端测试（`tui_pty_unix_test.go`）：光标移动、退格删除、CJK 输入

**新增测试：** 5 个（4 组件 + 1 PTY）  
**新增依赖：** `github.com/creack/pty`（仅测试用）

**修改文件：** `cmd/eliza/ui_events.go` (新增 248), `cmd/eliza/ui_components.go` (新增 230), `cmd/eliza/ui_components_test.go` (新增 94), `cmd/eliza/tui_pty_unix_test.go` (新增 80), `cmd/eliza/ui.go` (-85), `cmd/eliza/agent.go`, `go.mod`, `go.sum`

---

### 6. 输入体验全面优化

#### 6.1 Bracketed Paste 协议支持
...
**共计 15 个工具。** readonly 模式可用 10 个，autopilot 模式全开放。
