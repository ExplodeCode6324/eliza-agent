...
`memory` 工具支持 `recall`、`save`、`forget`。save/forget 会展示目标文件和精确新增/删除内容，并只接受本次 TUI 审批；拒绝、取消或非交互 `-q` 模式都不会写盘。审批使用上下键选择、回车确认；选择“拒绝，并告诉 ELIZA 应该怎么做”会把补充要求回传给 ELIZA。autopilot 和 `agent.md` 不能绕过审批。
