...
**最新更新 (2026-07-01):**
- **可选择式审批框** — 危险操作不再用 `/approve` `/deny` 斜杠命令，改为 `↑/↓` 选择 + `Enter` 确认的本地交互框。默认拒绝，批准只对本次生效。
- **无头浏览器支持 (06-30)** — 基于 Go chromedp，解压 Chrome Headless Shell 到 `~/eliza/tools/` 即用。详见 [无头浏览器](#无头浏览器) 章节。
...
| `input.go` | 终端输入（CJK 安全、粘贴检测、Backspace、审批导航） |
