...
仓库不含 Chrome 二进制（约 114M），请从 [Chrome for Testing](https://googlechromelabs.github.io/chrome-for-testing/) 官方源下载。项目测试版本为 **150.0.7871.24**，其他版本未验证。
...
# 或完整 Chrome（约 135M）
wget https://storage.googleapis.com/chrome-for-testing-public/150.0.7871.24/linux64/chrome-linux64.zip
```
...
内网部署时先在外网机器执行上述下载，再将压缩包和 eliza 二进制一同拷入内网解压。ELIZA 启动时自动扫描并激活浏览器工具。找不到 Chromium 时仅禁用浏览器工具，其他功能不受影响。
